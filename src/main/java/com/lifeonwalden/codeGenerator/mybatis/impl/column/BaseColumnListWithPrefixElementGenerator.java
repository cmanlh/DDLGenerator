...
public class BaseColumnListWithPrefixElementGenerator implements TableElementGenerator {
    ...
    public XmlElement getElement(Table table, Config config) {
        ...
        if (StringUtil.isNotBlank(alias)) {
            columnListText = BaseColumnListElementGenerator.setupColumnListText(table, alias.concat("."));
        } else {
            columnListText = BaseColumnListElementGenerator.setupColumnListText(table, "pre_".concat(table.getName().toLowerCase()).concat("."));
        }
