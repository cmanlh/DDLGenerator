...
public class DAOGeneratorImpl implements TableBasedGenerator {
    ...
    public String generate(Table table, Config config) {
        ...
        if (null != table.getPrimaryColumns()) {
            ...
            daoTypeBuilder.addMethod(MethodSpec.methodBuilder("updateDynamic").addModifiers(Modifier.PUBLIC, Modifier.ABSTRACT)
                            .returns(Integer.class).addParameter(resultBeanClass, "param").build());

            com.squareup.javapoet.MethodSpec.Builder getBuilder =
                            MethodSpec.methodBuilder("get").addModifiers(Modifier.PUBLIC, Modifier.ABSTRACT).addParameter(resultBeanClass, "param")
        ...
        try {
            JavaFileTmp.builder(config.getDaoInfo().getPackageName(), daoTypeBuilder.build())
                            .build()
