...
public class HashBeanGeneratorImpl extends BeanGeneratorImpl {
  ...
  private void methodBuild(Builder beanBuilder, ClassName className, Column column) {
    ...
    com.squareup.javapoet.MethodSpec.Builder getMethodBuilder =
        MethodSpec.methodBuilder("get" + StringUtil.firstAlphToUpper(StringUtil.removeUnderline(column.getName()))).addModifiers(Modifier.PUBLIC)
            .returns(javaTypeClassName)
            .addCode(CodeBlock.builder().addStatement("Object val = dataMap.get($S)", StringUtil.removeUnderline(column.getName()))
    ...
    com.squareup.javapoet.MethodSpec.Builder setMethodBuilder =
        MethodSpec.methodBuilder("set" + StringUtil.firstAlphToUpper(StringUtil.removeUnderline(column.getName()))).returns(className)
            .addModifiers(Modifier.PUBLIC).addParameter(javaTypeClassName, StringUtil.removeUnderline(column.getName()))
            .addStatement("dataMap.put($S,$L)", StringUtil.removeUnderline(column.getName()), StringUtil.removeUnderline(column.getName()))
