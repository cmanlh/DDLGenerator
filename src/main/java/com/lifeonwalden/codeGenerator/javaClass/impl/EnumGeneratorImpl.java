...
public class EnumGeneratorImpl implements EnumGenerator {
  ...
  public void generate(EnumConst enumConst, Config config) {
    ...
    enumTypeBuilder.addEnumConstant("NIL", TypeSpec.anonymousClassBuilder("$S, $L, $S, $S", "NIL", 0, "未知", "unknow")
        .addJavadoc("NULL").addJavadoc("\n").build());
    for (ValueEnum valueEnum : enumConst.getOptions()) {
      ...
      enumTypeBuilder.addEnumConstant(enumConstName, TypeSpec
          .anonymousClassBuilder("$S, $L, $S, $S", enumConstName, valueEnum.getValue(), enumConstDesc, enumConstAlias)
    ...
    enumTypeBuilder.addField(FieldSpec
        ...
        .initializer(CodeBlock.builder().add("new $T<$L, $L>()", HashMap.class, "Integer", enumConst.getName()).build())
        .build());
    ...
    enumTypeBuilder.addField(FieldSpec
        ...
        .initializer(CodeBlock.builder().add("new $T<$L, $L>()", HashMap.class, "String", enumConst.getName()).build())
        .build());
    ...
    enumTypeBuilder.addStaticBlock(
        CodeBlock.builder().addStatement("$L[] enumArray = $L.values()", enumConst.getName(), enumConst.getName())
    ...
    try {
      JavaFile.builder(config.getEnumInfo().getPackageName(), enumTypeBuilder.build()).build().writeTo(
