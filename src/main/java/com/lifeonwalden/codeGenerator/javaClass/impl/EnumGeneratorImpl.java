...
public class EnumGeneratorImpl implements ConstBasedGenerator {
  ...
  public String generate(EnumConst enumConst, Config config) {
    ...
    enumTypeBuilder.addEnumConstant("NIL",
        TypeSpec.anonymousClassBuilder("$S, $L, $S, $S", "NIL", 0, "未知", "unknow").addJavadoc("NULL").addJavadoc("\n").build());
    ...
    try {
      JavaFile.builder(config.getEnumInfo().getPackageName(), enumTypeBuilder.build()).build()
          .writeTo(new File(new File(config.getOutputLocation()).getPath() + File.separator + config.getEnumInfo().getFolderName()));
    } catch (IOException e) {
      e.printStackTrace();
    }
