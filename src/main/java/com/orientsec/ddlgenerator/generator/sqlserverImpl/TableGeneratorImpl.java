...
public class TableGeneratorImpl implements TableGenerator {
    ...
    public String generator(Table table, TableConfig config) {
        ...
        sb.append("--").append(table.getComment()).append(OutputUtil.LINE_SEPERATOR);
        sb.append("IF EXISTS (SELECT * FROM SYS.TABLES WHERE NAME = '").append(table.getName()).append("' AND TYPE = 'U'");
        ...
        sb.append("CREATE TABLE ").append(schemaPrefix).append(table.getName()).append("(").append(OutputUtil.LINE_SEPERATOR);
        List<Column> columnList = table.getColumn();
        sb.append(OutputUtil.INDENT_SIZE).append(columnGenerator.generator(columnList.get(0))).append(OutputUtil.LINE_SEPERATOR);
